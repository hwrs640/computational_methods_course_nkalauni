...
The model shows mild overfitting: training loss decreases steadily from $\sim$1.69 to $\sim$0.40 by epoch 80, whereas validation loss plateaus around 0.85--0.90 after epoch 50 while continuing to fluctuate. Validation mIoU improves through epoch 66, reaching a best of \textbf{0.3143}, then stagnates -- consistent with the model beginning to overfit while the correctly computed mIoU (averaged over all 11 classes) leaves less room for apparent gain from skipping rare classes.
...
\[
    \text{mIoU} = \mathbf{0.3143}
\]

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{iou_per_class.png}
    \caption{Per-class IoU bar chart (produced inline in the notebook). The dashed red line marks the global mIoU of 0.3143.}
    \label{fig:iou_bar}
\end{figure}

The per-class breakdown (Figure~\ref{fig:iou_bar}) reveals a clear split. The two most frequent classes score highest: Trees (10, $\sim$70.6\% of pixels, IoU $\approx$ 0.83) and Open Water (80, $\sim$8.6\%, IoU $\approx$ 0.83). Cropland (40, $\sim$7.4\%, IoU $\approx$ 0.64) and Grassland (30, $\sim$9.3\%, IoU $\approx$ 0.41) also exceed the mIoU, while Built-up (50, $\sim$3.8\%, IoU $\approx$ 0.49) performs reasonably. The model struggles most with Bare/sparse vegetation (60, $\sim$0.18\%, IoU $\approx$ 0.12) and the four extreme minority classes -- Shrubland (20), Snow/Ice (70), Herbaceous Wetland (90), and Moss/Lichen (100) -- which all score near zero. This directly mirrors the class frequencies from Problem 1: despite inverse-frequency weighting, classes with fewer than $\sim$0.2\% of pixels provide too little signal for the model to learn reliable decision boundaries, and their near-zero IoU is the primary driver pulling the global mIoU below 0.32.
